...
A run succeeds without ever leaving normal mode when steps 1 to 4 between them
...
once, and deletes the task. This is the standard technique, needing only the
...
**headless**: it executes steps 1 to 4, registers any boot-time resume, and
writes the report to a file under `%ProgramData%\Wixen\` that the interactive
process reads back and shows. While it runs, the interactive process shows an
...
(the scheduled task cannot be created or run, or it produces no readable
result), the interactive process runs the removal in-process under
Administrator, with the live progress bar, exactly as it does without SYSTEM.
The re-launched instance takes an `--execute <product>` branch that only runs
the removal and never opens the menu, so a SYSTEM run can never spawn another.
...
The state format is plain, hand-written key/value text, with no serialization
dependency (consistent with the crate's no-dependencies promise). Its parser
round-trips under test, including a corrupt-file case that resumes to a safe
"nothing pending" rather than panicking.
